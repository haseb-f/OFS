...
import { mockImportSourceConfigs, mockVendorOBReviewRecords } from '@/lib/mock-data';
import { fNum } from '@/lib/format';

const SOURCE_CONFIG = mockImportSourceConfigs.find(s => s.moduleType === 'vendor_opening_balances')!;

const INITIAL_RECORDS: ReviewRecord[] = mockVendorOBReviewRecords.map(r => ({
  id: r.id,
...
const COLUMNS: ColumnDef[] = [
  { key: 'vendor_name',  labelAr: 'المورد', width: 200 },
  ...
  return (
    <>
      <div className="page-header">
        <div>
          <h2 className="page-title">أرصدة افتتاحية — موردون</h2>
          <p className="page-subtitle">مراجعة واعتماد الأرصدة الافتتاحية للموردين</p>
        </div>
        <div style={{ display: 'flex', gap: 'var(--space-2)', alignItems: 'center' }}>
          <span style={{ fontSize: 'var(--font-size-xs)', padding: '3px 10px', borderRadius: 'var(--radius-full)', background: '#fee2e2', color: '#b91c1c', fontVariantNumeric: 'tabular-nums' }}>
            إجمالي مستحق: {fNum(totalCredit)} ر.س
          </span>

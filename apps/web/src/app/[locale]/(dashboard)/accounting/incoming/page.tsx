...
const SOURCE_TYPES: IncomingSourceType[] = [
  'customer_collection', 'bank_deposit', 'wallet_deposit', 'payment_gateway', 'other_income',
];
...
export default function IncomingOperationsPage() {
  const [records, setRecords]         = useState<OperationRecord[]>(mockIncomingOperations);
  const [selected, setSelected]       = useState<Set<string>>(new Set());
  const [expanded, setExpanded]       = useState<Set<string>>(new Set());
  const [search, setSearch]           = useState('');
  const [filterPosting, setFilterPosting] = useState<OperationPostingStatus | ''>('');
  const [filterSource, setFilterSource]   = useState<IncomingSourceType | ''>('');
  ...
  function handlePost(ids: string[]) {
    ...
    setTimeout(() => {
      setRecords(prev => prev.map(r => ids.includes(r.id) && r.postingStatus === 'posting'
        ? { ...r, postingStatus: 'posted', postedAt: '2026-05-24', postedBy: 'المستخدم الحالي', journalEntryId: `JE-2026-${1200 + Math.floor(Math.random() * 100)}` }
        : r
      ));
    }, 1800);
  }
  ...
  return (
    <>
      ...
      <div className="page-header">
        <div>
          <h2 className="page-title">مركز العمليات الواردة</h2>
          <p className="page-subtitle">مراجعة وترحيل المقبوضات والإيرادات المستوردة</p>
        </div>
      ...
      <div style={{ display: 'flex', gap: 'var(--space-4)', flexWrap: 'wrap', marginBlockEnd: 'var(--space-5)' }}>
        ...
        <SummaryCard label="إجمالي الواردات" value={`${fNum(stats.totalAmount)} SAR`} color="var(--color-primary)" />
      ...
      <div className="ofs-card" style={{ overflow: 'hidden' }}>
        {/* Toolbar */}
        <div className="ofs-ops-toolbar" style={{ background: selectedOnView.length > 0 ? '#f0fdf4' : 'transparent', transition: 'background 0.15s' }}>
          {/* Filters */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)', flexWrap: 'wrap' }}>
            ...
            />
            <OfsSelect
              options={[{ value: '', label: 'كل أنواع المصادر' }, ...SOURCE_TYPES.map(s => ({ value: s, label: INCOMING_SOURCE_LABELS[s] }))]}
              value={filterSource}
              onChange={v => setFilterSource(v as IncomingSourceType | '')}
              size="sm"
            />
          </div>
          ...
          <table className="ofs-table">
            <thead>
              <tr>
                ...
                <th style={{ inlineSize: 130 }}>نوع العملية</th>
                <th>الطرف / المرجع</th>
            ...
            <tbody>
              ...
              {filtered.map(record => {
                ...
                return (
                  <>
                    <tr key={record.id} style={{ background: isSelected ? '#f0fdf4' : undefined }}>
                      ...
                      <td>
                        <span style={{ fontSize: 'var(--font-size-xs)', padding: '2px 7px', borderRadius: 'var(--radius-full)', background: 'var(--color-surface-raised)', color: 'var(--color-text-muted)', border: '1px solid var(--color-border)', whiteSpace: 'nowrap' }}>
                          {INCOMING_SOURCE_LABELS[record.sourceType as IncomingSourceType]}
                        </span>
                      </td>
        ...
        {/* Footer — post all unposted approved */}

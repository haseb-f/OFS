...
const TAG = '[/api/auth/platform-login]';
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const endpoint = `${API_URL}/v1/auth/platform-login`;
  ...
  console.log(`${TAG} Auth success — userId: ${data.userId} | role: ${data.roleCode}`);
  ...
  session.roleCode   = 'PLATFORM_OWNER';
